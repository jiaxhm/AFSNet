...
import models
# from models.dual import DUAL
import datasets
from loss import compute_semantic_pos_loss, cross_entropy_loss_edge, dice
import datetime
...
#     api_key="fQ0kz3NspYVSSfWzZXOiccGd7",
#     project_name="general1",
#     workspace="wczxxy",
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='vit_base',  help='model architecture')
# parser.add_argument('--data', metavar='DIR',default='/root/autodl-tmp/data_preprocessing/DUMP_DIR/', help='path to input dataset')
parser.add_argument('--data', metavar='DIR',default='E:\BSDS500\BSDS500_ESNet', help='path to input dataset')
# parser.add_argument('--savepath',default='/mnt/FCN/CKPT_LOG_DIR/', help='path to save ckpt')
...
parser.add_argument('--epochs', default=300000, type=int, metavar='N', help='number of total epoches, make it big enough to follow the iteration maxmium')
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, edge) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            print(
                'train Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_sem {6}\t Loss_pos {7}\t Loss_edge {8}'
