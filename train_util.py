...
def init_spixel_grid(args,  b_train=True):
    ...
    n_spixl_h = int(np.floor(img_height/args.downsize))#numpy.floor向下取整
    n_spixl_w = int(np.floor(img_width/args.downsize))#计算超像素个数

    spixel_height = int(img_height / (1. * n_spixl_h))
    spixel_width = int(img_width / (1. * n_spixl_w))#确定超像素宽高

    spix_values = np.int32(np.arange(0, n_spixl_w * n_spixl_h).reshape((n_spixl_h, n_spixl_w)))#超像素的值为h行w列的二维矩阵，矩阵内值为0到n_spixl_w * n_spixl_h
    spix_idx_tensor_ = shift9pos(spix_values)

    spix_idx_tensor =  np.repeat(
        np.repeat(spix_idx_tensor_, spixel_height,axis=1), spixel_width, axis=2)#np.repeat()用于将numpy数组重复。列上有多少超像素就在列维度复制多少次，行维度同理
    torch_spix_idx_tensor = torch.from_numpy(
                np.tile(spix_idx_tensor, (args.batch_size, 1, 1, 1))).type(torch.float).cuda()#将spix_idx_tensor按行复制扩展batch_size次，然后利用torch.from_numpy转换为张量送入gpu
#转为bchw
    ...
    # pixel coord像素坐标
    all_h_coords = np.arange(0, curr_img_height, 1)#从0到curr_img_height按步长为1生成等差数列
    all_w_coords = np.arange(0, curr_img_width, 1)
    curr_pxl_coord = np.array(np.meshgrid(all_h_coords, all_w_coords, indexing='ij'))#根据图像宽高等差数列生成坐标

    coord_tensor = np.concatenate([curr_pxl_coord[1:2, :, :], curr_pxl_coord[:1, :, :]])

    all_XY_feat = (torch.from_numpy(
        np.tile(coord_tensor, (args.batch_size, 1, 1, 1)).astype(np.float32)).cuda())#bchw每个像素赋予坐标
...
def shift9pos(input, h_shift_unit=1,  w_shift_unit=1):
    ...
    input_pd = np.pad(input, ((h_shift_unit, h_shift_unit), (w_shift_unit, w_shift_unit)), mode='edge')#将输入矩阵上下前后各填充h_shift_unit，w_shift_unit个值，用边缘值填充
    input_pd = np.expand_dims(input_pd, axis=0)#np.expand_dims()二维变三维
    ...
    shift_tensor = np.concatenate([     top_left,    top,      top_right,
                                        left,        center,      right,
                                        bottom_left, bottom,    bottom_right], axis=0)#np.concatenate 是numpy中对array进行拼接的函数
    return shift_tensor
...
def poolfeat(input, prob, sp_h=2, sp_w=2):
    ...
    def feat_prob_sum(feat_sum, prob_sum, shift_feat):
        feat_sum += shift_feat[:, :-1, :, :]#切片
        prob_sum += shift_feat[:, -1:, :, :]#切片
    ...
    feat_ = torch.cat([input, torch.ones([b, 1, h, w]).cuda()], dim=1)  # b* (n+1) *h*w，通过torch.cat将输入和torch.ones生成的四维向量在列维度上拼接
    prob_feat = F.avg_pool2d(feat_ * prob.narrow(1, 0, 1), kernel_size=(sp_h, sp_w),stride=(sp_h, sp_w)) # b * (n+1) * h* w，torch.narrow取第一列的元素，F.avg_pool2d()数据是四维输入,二维平均池化操作,将
...
def upfeat(input, prob, up_h=2, up_w=2):
    ...
    # prob b*9*h*w
    b, c, h, w = input.shape#其中b表示输入的数量，c表示特征图的通道数,h、w分布表示特征图的高宽
    ...
    gt_frm_top_left = F.interpolate(feat_pd[:, :, :-2 * h_shift, :-2 * w_shift], size=(h * up_h, w * up_w),mode='nearest')#F.interpolate利用插值方法，对输入的张量数组进行上\下采样操作,这里是2倍上采样（最近邻插值模式）
...
def assign2uint8(assign):
    ...
    red = torch.cat([torch.ones(size=assign.shape),  torch.zeros(size=[b,2,h,w])],dim=1).cuda()#将两个张量在列维度上拼接
...
def update_spixl_map (spixl_map_idx_in, assig_map_in):
    ...
    b,_,h,w = assig_map.shape#b*9*h*w
...
##################################################################################################
    # points_cpu = spixl_map_idx.to(device='cpu')
    # import matplotlib.pyplot as plt
    # import time
    # image_tensor = points_cpu[0]
    ...
    # show(make_grid(points_cpu, nrow=5, padding=10, pad_value=0))
    # plt.show()

    #################################################################################################

    assig_max,_ = torch.max(assig_map, dim=1, keepdim= True)#取概率最大的，其他原有维度不变
    assignment_ = torch.where(assig_map == assig_max, torch.ones(assig_map.shape).cuda(),torch.zeros(assig_map.shape).cuda())#assig_map == assig_max返回一个b*1*h*w的全1张量，否则返回全0张量
    new_spixl_map_ = spixl_map_idx * assignment_ # winner take all  spixl_map_idx对应每个像素映射的九个网格，乘以assignment_得到最终的映射
    new_spixl_map = torch.sum(new_spixl_map_,dim=1,keepdim=True).type(torch.int)#按行维度进行加和，保持原有维度不变
    #return assig_max
    return new_spixl_map#b*1*h*w


def get_spixel_image(given_img, spix_index, n_spixels = 600, b_enforce_connect = False):

    if not isinstance(given_img, np.ndarray):
        given_img_np_ = given_img.detach().cpu().numpy().transpose(1,2,0)#detach(): 返回一个新的Tensor，但返回的结果是没有梯度的。cpu():把gpu上的数据转到cpu上。numpy():将tensor格式转为numpy。transpose将第二维度挪到第一维上，第三维移到第二维上，原本的第一维移动到第三维上
    ...
    given_img_np = cv2.resize(given_img_np_, dsize=(w, h), interpolation=cv2.INTER_CUBIC)#将given_img_np_大小变为h*w,采用三次插值法(cv2.INTER_CUBIC)

    if b_enforce_connect:
        spix_index_np = spix_index_np.astype(np.int64)
        segment_size = (given_img_np_.shape[0] * given_img_np_.shape[1]) / (int(n_spixels) * 1.0)#（given_img_np_行维度*列维度）/超像素个数=分割大小
        min_size = int(0.06 * segment_size)
        max_size =  int(3 * segment_size)
        spix_index_np = enforce_connectivity(spix_index_np[None, :, :], min_size, max_size)[0]#标签数组
    cur_max = np.max(given_img_np)#取图像数组中的最大值
    spixel_bd_image = mark_boundaries(given_img_np/cur_max, spix_index_np.astype(int), color = (1,1,0)) #cyna#标记边界，颜色设置为（0,1,1） given_img_np/cur_max归一化
...
def spixlIdx(args, b_train = False):
    ...
    spix_values = np.int32(np.arange(0, n_spixl_w * n_spixl_h).reshape((n_spixl_h, n_spixl_w)))#创建一个h*w的0到超像素大小的矩阵
    spix_idx_tensor = shift9pos(spix_values)#转换为张量?

    torch_spix_idx_tensor = torch.from_numpy(
        np.tile(spix_idx_tensor, (args.batch_size, 1, 1, 1))).type(torch.float).cuda()#将spix_idx_tensor在指定维度上扩展，再转换为张量
...
def batch2img(img):
    b,_,h,w = img.shape
    tmp = img.permute(0,2,3,1)#变维b*h*w*c
    for i in range(b):
        if i ==0:
            tmp_stack = tmp[i,:,:,:]
        else:
...
def rgb2Lab_torch(img_in, mean_values = None):#rgb转换为LAB
    # self implemented function that convert RGB image to LAB
    # input img intense should be [0,1] float b*3*h*w
    assert img_in.min() >= 0 and img_in.max() <= 1

    img= (img_in.clone() + mean_values.cuda()).clamp(0, 1)#将输入input张量每个元素的夹紧到区间(0,1)

    mask = img > 0.04045
    img[mask] = torch.pow((img[mask] + 0.055) / 1.055, 2.4)#torch.pow对输入分量进行求幂操作（2.4次幂）
    img[~mask] /= 12.92
    ...
    xyz_img = torch.matmul(rgb, xyz_from_rgb.transpose_(0,1))#torch.matmul矩阵乘法
